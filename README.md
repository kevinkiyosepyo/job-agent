...
- `fixture_e2e.py` — non-submitting vertical fixture flows for supported ATSs, including CGI/Njoyn resume attachment verification, parsed-profile review evidence, and sanitized confirmation artifacts. Explicit parsed-profile mismatches emit blocked, field-only parser-repair evidence until a correction is recorded; the flows never submit, write a tracker row, or send a notification.
