...
- `ats_registry.py` / `prepare_job.py` — shared non-submitting dispatcher for Greenhouse, Workday, Lever, Oracle, and CGI/Njoyn saved surfaces. Every payload explicitly carries `submission_enabled: false`; manual-gated and listing surfaces remain fail-closed. A matching learned tenant record can carry only a runtime-only session reference and authenticated-state metadata into a plan, allowing a repeat flow to reuse an already authenticated session without serializing credentials or scheduling account creation.
