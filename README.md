...
- `cdp_page_executor.py` — bounded exact-target action executor. It requires a fresh trusted read-only CDP snapshot whose target ID and URL exactly match the requested page before it delegates an action contract; stale or untrusted state blocks before mutation, and returned evidence includes the verified target binding.
