...
- `njoyn_handler.py` — non-mutating CGI/Njoyn surface inspector; inventories listing identity/Apply entrypoints, account email/password controls, privacy-notice acknowledgement controls, employment-disclosure radios, voluntary-disability selects, and resume-upload controls with attached-filename read-back verification, while treating account sign-in/profile creation, privacy notices, required employment disclosures, and voluntary disability disclosures as fail-closed manual gates until a supported application surface is recognized.
