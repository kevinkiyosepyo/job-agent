...
- `njoyn_handler.py` — non-mutating CGI/Njoyn surface inspector; inventories listing identity/Apply entrypoints, account email/password controls, privacy-notice acknowledgement controls, employment-disclosure radios, voluntary-disability selects, resume-upload controls with attached-filename read-back verification, and explicit parsed-profile mismatch markers. It reports parser correction requirements without silently accepting parsed data, while treating account sign-in/profile creation, privacy notices, required employment disclosures, voluntary disability disclosures, and parser corrections as fail-closed manual gates until explicitly handled.
