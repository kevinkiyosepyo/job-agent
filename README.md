...
- `njoyn_handler.py` — non-mutating CGI/Njoyn surface inspector; inventories listing identity/Apply entrypoints, account email/password controls, and privacy-notice acknowledgement controls, while treating account sign-in/profile creation and privacy disclosure as fail-closed manual gates until a supported application surface is recognized.
