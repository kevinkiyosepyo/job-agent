...
- `prepare_live_job.py` — exact-target, non-submitting live-preparation seam. It requires fresh trusted page target/URL read-back, exact company/role/requisition equality, handler question inventory, and sanitized answer-coverage evidence before declaring a Review-ready bundle; target or identity drift fails before handler dispatch.
