...
- `browser_actions.py` — deterministic browser-action contracts for text replacement, native-option selection, radio/checkbox state changes, CDP file attachment, and scroll-and-click; each requires exact post-action read-back evidence before an action is considered verified.
