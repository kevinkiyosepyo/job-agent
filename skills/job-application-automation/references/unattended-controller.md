...
When configuring the repository controller, read `~/Documents/job-agent/docs/AUTONOMOUS-OPERATION.md` and the actual CLI/source before enabling it. `autonomous_config.example.json` intentionally has `production_enabled: false`; copying it does not authorize or start applications.
